...
st.write("See how the Tax Relief for American Families and Workers Act would change your Child Tax Credit from 2023 to 2025. Powered by PolicyEngine.")
...
st.write("Note: The US counts children as eligible for the Child Tax Credit if they are younger than 17 years of age.")

# Collecting the number of CTC-eligible children
ctc_eligible_children = st.number_input("How many eligible children do you have?", min_value=0, max_value=10, value=0)
...
def get_household_info(year, is_married, ctc_eligible_children, earned_income_data):
    situation = {
        "people": {
            "you": {
                "age": {str(year): DEFAULT_ADULT_AGE},
                "employment_income": {str(year): earned_income_data[year]},
                "employment_income_last_year": {str(year): 0 if year == 2023 else earned_income_data[year - 1]},
            }
        },
        "tax_units": {
            "your tax unit": {
    ...
    baseline_tax = baseline.calculate("income_tax_before_refundable_credits", year)[0]
    baseline_actc = baseline.calculate("refundable_ctc", year)[0]
    

    reform_simulation = Simulation(reform=reform, situation=situation)
    reform_tax = reform_simulation.calculate("income_tax_before_refundable_credits", year)[0]
...
for year in range(2023, 2026):
    income_change = get_household_info(year, is_married, ctc_eligible_children, earned_income_data)
...
df_income_changes = pd.DataFrame(list(income_changes.items()), columns=['Year', 'Income Change'])
...
fig = px.bar(df_income_changes, x='Year', y='Income Change', text='Income Change',
